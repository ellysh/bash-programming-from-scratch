...
Let us take an example. Imagine that the mainframe is running computational programs sequentially. The data for them are read from a magnetic tape and the results are printed on a printer. The OS loads each program and executes its instructions, then downloads the next one, and so on. Problems occur at the stages of reading the data and printing the result. The time to access data on the magnetic tape is huge on the CPU scale. Between two consecutive reading operations, it could perform a significant amount of computation. But it does not do this because all the computer resources are used only by the current program, which is currently loaded into memory. The same happens with the output of the calculation results to print. The printer, like a purely mechanical device, is very slow. This downtime problem led to the idea of [**multiprogramming**](https://en.wikipedia.org/wiki/Computer_multitasking#Multiprogramming).

Multi-programming involves loading several programs into your computer memory at the same time. The first program starts running as long as all the resources it needs are available. Once one of the resources is busy, it stops running. For example, the program needs the data stored on your hard disk. As long as the controller reads the first part of it, it is busy and cannot process the request to read the next part. In this case, the OS stops executing the first program and switches to the second part. In its turn, it is executed until the end of the program or until the time when the required resource is busy. After that, it switches back to the second part.

Multi-programming has become an early prototype of multitasking, which is implemented in all modern operating systems. Multiprogramming is good in batch processing mode. However, this load balancing approach is completely unsuitable for systems with [**interactive interaction**](https://en.wikipedia.org/wiki/Interactivity#Computing_science). On them, every user action (e.g. pressing a key) is an event that requires some kind of response from the system (e.g. adding a new character to a text document). The processing of events has to take place with minimum delays, otherwise the user will notice the hangs of the running program. The problem with multi-programming is that the next task is performed unpredictably long. As long as the current program has not been executed to the end or a busy resource has not been requested, task switching will not take place and no user actions can be processed.

Multitasking solves the problem of quick response when working interactively with a computer. The way it was implemented gradually developed and became more complicated. Modern operating systems use [**preemptive multitasking**](https://en.wikipedia.org/wiki/Preemption_(computing)) with pseudo-parallel processing of tasks. This means that it is up to the OS to decide which program to run at a given time. This choice takes into account the priorities of running applications. That is, hardware resources will be prioritized more often than low-priority ones. The task switching mechanism is implemented in the OS kernel and is called [**task scheduler**](https://en.wikipedia.org/wiki/Scheduling_(computing)).
