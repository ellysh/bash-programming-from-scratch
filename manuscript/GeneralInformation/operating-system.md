...
### Prerequisites for OS origin

Most computer users understand why [**operating system**](https://en.wikipedia.org/wiki/Operating_system) (OS) is needed. Generally, when you buy or download an application from the Internet, you check its system requirements. These specify, in addition to the hardware requirements, the operating system on which the application is running. It turns out that the OS is some software platform, which is necessary for the application to work. But where did this requirement come from? Why can't you just buy a computer and run an application on it without an OS at all?

These questions seem meaningless only at first sight. Think about it: modern operating systems are universal and offer the user many functions. Most of them simply do not need to be used by each specific user. But these functions are often impossible to disable. To maintain them, the OS actively uses computer resources. As a result, for the payload of the system, that is, applications with which the user works, there are much less resources. This leads to slow work, hangs and even reboots of the computer.

Let's look at the history to find out why the operating system was created. In fact, the first OS [GM-NAA I/O](https://en.wikipedia.org/wiki/GM-NAA_I/O) only appeared in 1956 for the computer [IBM 704](https://en.wikipedia.org/wiki/IBM_704). More and more earlier computer models did without an OS. Why wasn't there a need in them?

The main reason is the speed. For example, consider the first [**electromechanical computer**](https://en.wikipedia.org/wiki/Tabulating_machine#1890_census) designed by [Herman Hollerith](https://en.wikipedia.org/wiki/Herman_Hollerith) in 1890. This computer, called a tabulator, did not require an OS and [program](https://en.wikipedia.org/wiki/Computer_program) in the modern sense. The tabulator performed only a limited set of arithmetic operations, which was determined by its design. The data for calculations were downloaded from [**punched cards**](https://en.wikipedia.org/wiki/Punched_card), which represented the sheets of thick paper with punched holes. These sheets were manually prepared and stacked in special receivers. There they were threaded on the needles and in the holes there was a short circuit. Each short circuit led to an increase in the mechanical counter, which was a rotating cylinder. The calculation results were displayed on dials that resembled watches.

Figure 1-1 shows a tabulator built by Hermann Hallerith.
...
By modern standards, the tabulator worked very slowly. There were several reasons for this. First of all, the data for the calculations were prepared manually. There was no way to automatically punch through the cards. Then the loading of punch cards into the computer was also done manually. The tabulator itself contained a large number of mechanical parts: needles to read data, counters from rotating cylinders, dials to output the result. All this mechanics worked slowly. It took about one second to perform one elementary operation. No automation could have accelerated these processes.

Tabs working on rotating cylinders were replaced by computers using [**relay**](https://en.wikipedia.org/wiki/Relay). A relay is a mechanical element that changes its state due to electric current. One of [the first relay computers](https://en.wikipedia.org/wiki/Z2_(computer)), called Z2, was designed by a German engineer [Konrad Zuse](https://en.wikipedia.org/wiki/Konrad_Zuse) in 1939. Then this computer was improved in 1941 and was named Z3. By switching to a relay, the time for each elementary operation was reduced from a second to milliseconds.

In addition to the increased speed of computation, Zuse's computers had one more feature. The concept of a program appeared in them. Now with the help of punched cards we entered not the original data of the task, but [**algorithms**](https://en.wikipedia.org/wiki/Algorithm) by which it was solved. To enter the data, we used a keyboard remotely resembling a typewriter. These computers became known as [**programmable**](https://en.wikipedia.org/wiki/General_purpose_computer) or universal.
...
Often, programs were executed one after another without delay to avoid equipment downtime. Special solutions were needed to automate program loading and output. The first GM-NAA I/O operating system was developed precisely to manage program execution.
